...
but cannot be accessed outside its package.
...
of its subclasses
    ...
    new Base().display();
    -> not allowed because the caller's (this instance) class is not defined in the same package as the Base class,
...
define behaviours, and D as the subclass has the obligation to accept and conform.
    ...
    For easier understanding, the rule should really be simplified to require object's (static) type to be exactly S.
